...
export class UserResolver {
  ...
  async MobileSignUp(
    ...
    switch (role) {
      ...
      case 'parent':
      // return await this.userService.MobileParentSignUp(body);
      case 'business':
      // return await this.userService.MobileBusinessSignUp(body);
      default:
  ...
  async WebSignUp(
    ...
    switch (role) {
      case 'user':
      // return await this.userService.WebUserSignUp(body);
      case 'parent':
      // return await this.userService.WebParentSignUp(body);
      case 'business':
      // return await this.userService.WebBusinessSignUp(body);
      default:
        throw new Error('Invalid role');
    }
  }
  ...
  @Mutation(() => String)
  async test2(@Args('eq') eq: string): Promise<string> {
    return await this.userService.testQueue(eq);
  }
}
